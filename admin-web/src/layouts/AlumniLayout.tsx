        ...
        <header className="h-16 shrink-0 bg-white border-b border-[#E5E7EB] px-3.5 sm:px-8 flex items-center justify-between shadow-2xs z-20">
          <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-extrabold text-xs sm:text-base text-[#111111] tracking-tight truncate max-w-[150px] xs:max-w-[200px] sm:max-w-md">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-xs font-semibold transition-all cursor-pointer"
            >
        ...
        <main className="flex-1 flex items-center justify-center p-3.5 sm:p-6 md:p-10">
          <div className="max-w-2xl w-full bg-white border border-[#E5E7EB] rounded-3xl p-5 sm:p-10 shadow-xl space-y-5 sm:space-y-6 text-center animate-fadeIn relative overflow-hidden">
            ...
            <div className="flex justify-center">
              {isPending && (
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-[#FFF7D6] border-4 border-[#F4C542]/50 rounded-3xl flex items-center justify-center text-[#854D0E] shadow-xs">
                  <Clock className="w-8 h-8 sm:w-10 sm:h-10 text-[#854D0E] animate-pulse" />
                </div>
              )}
              {isRejected && (
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-rose-50 border-4 border-rose-200 rounded-3xl flex items-center justify-center text-rose-600 shadow-xs">
                  <AlertTriangle className="w-8 h-8 sm:w-10 sm:h-10 text-rose-600" />
                </div>
              )}
              {isSuspended && (
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-amber-50 border-4 border-amber-200 rounded-3xl flex items-center justify-center text-amber-600 shadow-xs">
            ...
            <div className="flex justify-center">
              {isPending && (
                <span className="bg-[#FFF7D6] text-[#854D0E] border border-[#F4C542]/70 px-3.5 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider flex items-center gap-2 text-center">
                  <span className="w-2 h-2 rounded-full bg-[#854D0E] animate-ping shrink-0" />
                  {language === 'ta' ? 'பள்ளி நிர்வாகியின் அனுமதி நிலுவையில் உள்ளது' : 'Awaiting School Admin Verification'}
                </span>
              )}
              {isRejected && (
                <span className="bg-rose-100 text-rose-800 border border-rose-300 px-3.5 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                  {language === 'ta' ? 'பதிவு நிராகரிக்கப்பட்டது' : 'Registration Rejected'}
                </span>
              )}
              {isSuspended && (
                <span className="bg-amber-100 text-amber-800 border border-amber-300 px-3.5 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider">
            ...
            <div className="space-y-2">
              <h2 className="text-xl sm:text-3xl font-extrabold text-[#111111] leading-tight">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 sm:p-5 text-left text-xs space-y-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <span className="font-bold text-[#111111] uppercase tracking-wider text-[11px] flex items-center">
                  <GraduationCap className="w-4 h-4 mr-1.5 text-[#854D0E] shrink-0" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 text-gray-700">
                <div>
                  <span className="text-gray-400 block text-[11px]">{language === 'ta' ? 'பெயர்:' : 'Full Name:'}</span>
                  <span className="font-semibold text-[#111111] truncate block">{user?.full_name || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block text-[11px]">{language === 'ta' ? 'படித்த வகுப்புத் தொகுதி:' : 'Batch Year:'}</span>
                  <span className="font-semibold text-[#111111] truncate block">Batch of {user?.passing_year || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block text-[11px]">{language === 'ta' ? 'மின்னஞ்சல்:' : 'Email Address:'}</span>
                  <span className="font-semibold text-[#111111] truncate block">{user?.email || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block text-[11px]">{language === 'ta' ? 'கைபேசி எண்:' : 'Mobile Number:'}</span>
                  <span className="font-semibold text-[#111111] truncate block">{user?.mobile || 'N/A'}</span>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="h-screen w-screen bg-[#FAFAFA] text-[#111111] flex flex-col overflow-hidden font-sans">
      ...
      <header className="h-16 shrink-0 bg-white border-b border-[#E5E7EB] px-3 sm:px-6 z-30 flex items-center justify-between shadow-2xs">
        ...
        <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
          <button
            ...
            className="lg:hidden p-2 rounded-xl bg-[#F3F4F6] text-[#111111] hover:bg-[#E5E7EB] cursor-pointer shrink-0"
          ...
          <button
            ...
            className="hidden lg:flex p-2 rounded-xl bg-[#FAFAFA] border border-[#E5E7EB] text-[#4B5563] hover:text-[#111111] hover:bg-[#F3F4F6] transition-all cursor-pointer shrink-0"
            title={sidebarCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
          >
            {sidebarCollapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}
          </button>

          <div className="flex items-center space-x-2.5 cursor-pointer select-none min-w-0" onClick={() => navigate('/alumni')}>
            <img
              src={school?.logo_url || logoUrl}
              alt="School Logo"
            ...
            <div className="hidden sm:block min-w-0">
              <div className="flex items-center space-x-1.5 min-w-0">
                <h1 className="font-extrabold text-xs sm:text-sm text-[#111111] truncate">{t('app_title')}</h1>
                <CheckCircle2 className="w-4 h-4 text-amber-500 fill-amber-100 shrink-0" />
              </div>
              <span className="text-[10px] sm:text-[11px] text-[#6B7280] block truncate">{t('alumni_class_of')} {user?.passing_year || 'Alumni'}</span>
            </div>
          </div>
        </div>

        {/* Middle: Global Header Search Bar */}
        <form onSubmit={handleSearchSubmit} className="hidden md:flex items-center relative w-48 lg:w-96">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
          ...
          <div className="flex items-center space-x-2.5 sm:space-x-3 pl-2 sm:pl-3 border-l border-[#E5E7EB]">
            ...
            <div className="hidden sm:block text-left cursor-pointer min-w-0" onClick={() => navigate('/alumni/profile')}>
              ...
              {isApproved ? (
                <span className="text-[10px] text-[#854D0E] font-semibold bg-[#FFF7D6] px-1.5 py-0.2 rounded-full inline-block truncate">{t('alumni_verified_badge')}</span>
              ) : (
                <span className="text-[10px] text-[#854D0E] font-bold bg-[#FFF7D6] border border-[#F4C542]/60 px-1.5 py-0.2 rounded-full animate-pulse inline-block truncate">
      ...
      <div className="flex-1 flex min-h-0 min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 h-full min-w-0 overflow-y-auto p-3.5 sm:p-6 lg:p-8 bg-[#FAFAFA]">
          {/* Awaiting Admin Verification Top Banner Bar */}
          {!isApproved && (
            <div className="mb-4 sm:mb-6 p-3.5 sm:p-5 bg-gradient-to-r from-[#FFF7D6] via-amber-50 to-[#FFF7D6] border-2 border-[#F4C542] rounded-2xl shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 animate-fadeIn">
              <div className="flex items-start sm:items-center space-x-3 min-w-0">
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-[#F4C542] rounded-xl flex items-center justify-center text-[#854D0E] shrink-0 shadow-2xs mt-0.5 sm:mt-0">
                  <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-[#854D0E] animate-pulse" />
                </div>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <h3 className="font-extrabold text-xs sm:text-sm text-[#111111] truncate">
                      {language === 'ta' ? 'பள்ளி நிர்வாகியின் அனுமதி நிலுவையில் உள்ளது' : 'Awaiting School Admin Verification'}
                    </h3>
                    <span className="bg-[#854D0E] text-white text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
