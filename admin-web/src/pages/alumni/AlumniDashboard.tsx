    ...
    <div className="space-y-6 max-w-6xl mx-auto font-sans text-[#111111]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between text-gray-500 mb-3">
            <span className="text-xs font-semibold">Total Alumni</span>
            <Users className="w-4 h-4 text-amber-600" />
          </div>
          <p className="text-2xl font-extrabold text-[#111111]">{stats.total_alumni || alumniCount || 0}</p>
          <span className="text-[11px] text-[#6B7280] mt-1">Registered members</span>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between text-gray-500 mb-3">
            <span className="text-xs font-semibold">Graduating Batches</span>
            <Clock className="w-4 h-4 text-emerald-600" />
          </div>
          <p className="text-2xl font-extrabold text-[#111111]">{stats.total_batches || 0}</p>
          <span className="text-[11px] text-emerald-600 font-semibold mt-1">Batches connected</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm flex flex-col justify-between">
          ...
          <span className="text-[11px] text-blue-600 font-semibold mt-1">Active events</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm flex flex-col justify-between">
          ...
          <span className="text-[11px] text-[#6B7280] mt-1">Official notices</span>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-4 w-full">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-base text-[#111111]">Upcoming Alumni Events</h3>
              <p className="text-xs text-[#6B7280]">Register for upcoming gatherings and reunions</p>
            </div>
            <button
          ...
          <div className="space-y-3">
            {events.length > 0 ? (
              events.slice(0, 2).map(ev => (
                <div key={ev.id} className="p-4 rounded-xl bg-[#FAFAFA] border border-[#E5E7EB] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-bold text-xs sm:text-sm text-[#111111]">{ev.title}</h4>
              ...
              <div className="p-6 rounded-xl bg-[#FAFAFA] text-center border border-dashed border-[#E5E7EB]">
                ...
                <p className="text-xs text-[#6B7280]">No upcoming events scheduled right now.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-base text-[#111111]">Recent Announcements & Notices</h3>
            <p className="text-xs text-[#6B7280]">Official notices from school admin and alumni association</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {announcements.length > 0 ? (
            ...
            <div className="col-span-2 p-6 rounded-xl bg-[#FAFAFA] text-center border border-dashed border-[#E5E7EB]">
              ...
              <p className="text-xs text-[#6B7280]">No active announcements posted yet.</p>
            </div>
          )}
        </div>
      </div>
